...
Item {
  ...
  //
  // NOT `required`: a cloned bar loads via Loader.source (a URL), which
  // can't supply initial property values at construction the way the
  // built-in bar's sourceComponent can. configureBar() in shell.qml sets
  // these three immediately after construction instead. Workaround for
  // https://github.com/basecamp/omarchy/issues/8007, reported upstream as
  // fixed (properly, via Loader initialProperties) by
  // https://github.com/basecamp/omarchy/pull/8146. Once that PR ships in a
  // stable release, drop this comment and restore `required` on all three.
  ...
  // When floatGap isn't set explicitly, it's auto-detected from Hyprland's
  // own `general:gaps_out` (see gapsOutProbe below), so the bar's floating
  // margin matches the gap already used between windows and the screen
  // edge instead of an arbitrary constant. See applyBarConfig().
  ...
  component BarPanel: PanelWindow {
    ...
    // The edge the bar is anchored to (its `position`) and the two
    // perpendicular edges get the gap -- those all border open desktop or
    // screen edge. The edge opposite `position` gets none: that's the side
    // facing the tiled windows, which already have their own Hyprland gap
    // right up against the bar's exclusion zone, so adding floatGap there
    // on top of it just makes that side look bigger than the other three.
    function innerEdge() {
    ...
    function marginFor(edge) {
      return edge === innerEdge() ? 0 : root.floatGap
    }

    margins {
      top: marginFor("top") + (root.barHidden && root.position === "top" ? -root.barSize : 0)
      bottom: marginFor("bottom") + (root.barHidden && root.position === "bottom" ? -root.barSize : 0)
      left: marginFor("left") + (root.barHidden && root.position === "left" ? -root.barSize : 0)
      right: marginFor("right") + (root.barHidden && root.position === "right" ? -root.barSize : 0)
    }
    ...
    // the actual (roundable) fill, so the corners cut outside its radius
    // show through to the desktop instead of being a square window.
    color: "transparent"
